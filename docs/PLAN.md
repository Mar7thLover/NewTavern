...
| 项目 | 内容                                                                                                                                                                                                           |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 状态 | MVP（M0–M3）、M4、M5、M6 创作工作台、M7 长篇写作均已完成（2026-09-23 收尾轮续做，见 §四末「进度记录」）；M8 CRPG / 多智能体、M9 Tauri 未开始 |
| 版本 | v0.8（2026-09-23） |
...
| **M5 前端卡与酒馆助手**（约 3 周，主体已完成） | 沙箱与信任级别、RPC、状态镜像、酒馆助手 shim、事件映射、MVU 引擎、变量面板、脚本库（正文块与内联 HTML 前端＝M5（一）；运行时与 MVU＝M5（二））；剩下变量管理器 UI、EJS 模板、slash 全量 → M5（三） | 社区 Top 20 前端卡中 ≥ 15 张开箱可用；5 张流行 MVU 卡零修改可用                                                    |
...
| 2026-09-14 | M3       | 世界书引擎（ST 算法移植、按节点快照的时间态）、正则引擎、宏 v2、变量事务；组装流水线 v2 与 strict / cache-aware 布局器；62 组 CC0 合成用例 + 驱动本机 ST 1.18 的录制工具，黄金测试 62/62 逐字节一致（目标 30）；服务端接入（快照/变量落库、inspect、compare）；检查器、作者注释、世界书绑定与设置、全局系统提示词、正则管理、显示侧正则；真机 20 轮 cache-aware 平均缓存命中率 81.7%（目标 ≥70%）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 视觉体系按 `docs/DESIGN.md` v2 完成：主题引擎（语义槽位、材质类、data-part 挂点、记忆物件注入、主题选项、@scope 隔离）+ 六个独立世界（素/琉璃/书斋/酒馆/雨夜/暖房）全部实现并逐张审阅通过（2026-09-14）；生成中动画、减弱动效与真机性能未实拍验证；用户反馈后提前做：预设基础编辑器（原属 M6，名称/采样/条目开关排序正文注入，导出无损）、开发者模式下的「ST 对照」（完整差异列表）、默认用户档案与档案字段对齐 ST persona（头像、称号、描述位置 in_prompt/at_depth/top_an/bottom_an/none、绑定世界书，黄金测试仍 62/62）；ST persona connections 与导入、`{{persona}}` 在角色卡字段中展开为空的旧问题待处理；「导入 10 张主流卡 + 5 个主流预设正常对话」仅用本机 8 张卡手动抽验，未系统化 |
| 2026-09-16 | M4（一） | ST 迁移收尾：聊天 jsonl ↔ 消息树（swipe/分支/隐藏/媒体，本机 12 份导入→导出 deep-equal）、对话列表导入导出、目录迁移向导（只允许本机；本机样本 43 项导入 0 失败）；多模态：上传（图片/PDF/文本，PDF 抽文本）、附件入树、文本附件按 ST 内联、四家适配器原生图片/PDF 渲染、落库请求脱敏、模型生图（Gemini / Responses 工具 / OpenRouter 形态）落库与 SSE、输入托盘、消息附件、灯箱、画廊、生图开关、资产清理；`{{persona}}` 等卡类宏按 ST 1.18 新宏引擎实录修正；消息列表虚拟化（2000 条首屏 5–7 个节点）、路由代码分割（首屏 996→563 KiB）、命令面板 Ctrl/⌘+K；节点与会话摘要载荷瘦身                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 多模态仅 mock 端点验证（Z.AI 无视觉模型）；Google / Responses 生图与 OpenRouter 流式 `delta.images` 形状未实测；未按提供商限制单图/单请求体积；文件文本会经过提示词正则与宏（ST 是先正则后拼文件）；群聊不迁移；M4（二）：背景、立绘表情、外接生图后端（SD / ComfyUI / NovelAI）、主题导入导出                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-09-17 | M5（一） | 正文块与内联前端（M5 的前一半）：主题无关的结构识别层（`packages/core/src/richtext/blocks.ts`，认状态栏 / 思考 / 选项 / 摘要 / 旁白 / 场外话 / 变量更新七类，标签式 + `【…】` 式 + 行内 `（OOC：…）`；流式中未闭合标签也能出块；围栏代码与卡自带的 `style/script` 段不穿透）→ 统一语义标记 `data-nt-block`；六个世界各写各的皮肤 `themes/<id>/blocks.css`（素＝发丝线与 tabular 数字、书斋＝双线框与竹尺朱砂、琉璃＝薄冰与冰棱、酒馆＝压进羊皮纸的凹格与黄铜刻度铆钉、雨夜＝湿玻璃与水位、暖房＝便签与软糖条），骨架默认形态在 `themes/blocks.css`；卡 / 世界书 / 预设自带的 HTML 前端内联渲染（rehype-raw + rehype-sanitize 白名单；`<style>` 用 `@scope` 关进本条消息、`:root`→`:scope`、`@keyframes` 按消息改名；`<script>`/`<iframe>`/事件属性/`javascript:` 全剥；`data:image/*` 放行）；设置 → 外观 →「正文」两个开关（主题美化 / 卡自带前端，默认开，都关掉即回到原来的纯 Markdown + 转义）；core 48 个单测 + web 16 个整链路渲染测试；10 个世界×模式在真机 Chrome 逐张截图审阅，并用 CDP 实测对比度与数值条几何（`scrollWidth` 无溢出，三处自定的弱对比已修） | 卡自带前端只做到「无脚本」：带 `<script>` 的前端卡仍要等 M5 的 iframe 沙箱（RPC、酒馆助手 shim、MVU）；识别层只认收录的标签与 `【…】` 小标题，社区自造的标签要用户写正则先转成这几类；状态栏的行解析只认冒号 / Markdown 表格行 / `                                                                                                                                                                                                                                                                                                                                                                                                                                                         | ` 并排，多层嵌套的表格面板会退化成纯文本块；`@scope` 需要 Chrome 118+ / Safari 17.4+ / Firefox 128+（主题引擎本来就依赖它）；rehype-raw 带进 parse5，对话路由的 markdown chunk +216 KB（gzip +63 KB）；未在真实社区前端卡上抽验（本机没有样本），未拍移动端 390 宽 |

| 2026-09-17 | M5（二） | **前端卡运行时**：识别层 `packages/core/src/richtext/cards.ts`（围栏里的 HTML 文档 / 带 `<script>` 的整段 → 一张卡；`js` 等明确语言的围栏不认）；沙箱 SDK `packages/sandbox-sdk`（RPC 协议与信封、`buildSrcdoc`、宿主帧通道、iframe 引导脚本）；四档信任级别（strict / standard / trusted / legacy-unsafe）对应 CSP 与 `sandbox` 属性，默认 standard：opaque origin、https 资源可加载、网络只能回宿主；本地打包 jQuery / lodash / Vue / zod / YAML（`scripts/build-sandbox-libs.mjs` 打成 IIFE 经典脚本，按需注入，脚本帧全给），内存版 localStorage polyfill；高度由 guest 的 `ResizeObserver` 报上来，镜像更新不重建 iframe。**酒馆助手兼容**：`window.newtavern.*` 原生 API + 全局函数 shim（消息 / 变量 / 事件 / 生成 / 世界书 / 脚本按钮 / `substitudeMacros` / `triggerSlash` 子集 / `errorCatched` / `waitGlobalInitialized`），同步 API 走状态镜像、写操作走 RPC；楼层号 ↔ 节点 id 的映射（每条消息额外带 `node_id`）；事件名表照抄 4.9.3，真的会广播的十几个列在契约 §5.3；未实现的接口**调用即明确报错**，不静默失效；脚本库（角色卡 `extensions.tavern_helper.scripts` / 旧字段）在隐藏 iframe 里跑，脚本按钮显示在输入框上方。**MVU**：服务端引擎 `packages/compat/src/mvu`（`[InitVar]` 初始化、`_.set/_.add/_.insert/_.remove` 与 `<JSONPatch>` 两种写法、VWD `[值,说明]` 二元组、`$meta.strictSet/extensible`、`display_data`/`delta_data`），生成流式结束后按节点快照应用并写回、SSE `variables` 事件；`{{get_*_variable::}}` / `{{format_*_variable::}}` 宏进组装（`$` 簿记键不进提示词）；重放（`mvu/replay`）与单条重算（`mvu/run`）；变量接口按五个作用域；检查器新增「变量」页签。**验证**：compat 31 + core 8 + sandbox-sdk 19 + web 10 + 服务端 5 个新测试；真机 Chrome 用本机两张真实社区卡（长夜月＝YAML InitVar + Vue 状态栏 + MVU/Zod 脚本；黄金庭院＝JSON `$meta` InitVar + `[值,说明]` + 角色面板）跑通：卡在沙箱里渲染、读到真实变量、脚本按钮出现、1440 与 390 宽均无横向溢出、关掉开关退回代码块；并用 mock 提供商实跑一轮生成，确认「生成 → MVU → SSE → 镜像 → 卡上的数值实时变化」整条链路。 | 变量管理器（可视化编辑 + `registerVariableSchema` 校验）只有只读面板；EJS 提示词模板（`<%_ _%>`）与酒馆助手宏开关未做；MVU 的额外模型解析与旧楼层变量自动清理未做；slash 只有 7 条常用命令；全局脚本库没有导入 / 编辑界面（只跑卡自带的脚本）；前端卡的 `generate` 不支持 `injects` / `overrides` / `tools` / `json_schema`（带了会回 warning）；`formatAsTavernRegexedString` 原样返回；`SillyTavern.*` 只有六个字段；主题槽位还没真的喂进卡（通道已打通）；只在本机两张真卡上验证过，社区 Top 20 的覆盖率未系统抽验。 |

| 2026-09-18 | M5（二·补） | **自带正则收进正则库**（用户反馈：卡 / 预设 / 世界书带的正则应当默认导入到正则脚本处、并询问是否加载）：`regex_scripts.scope` 扩成 `global | character | preset | book`，导入时把 `extensions.regex_scripts` 抽进表（原件不动，导出仍无损），默认不启用；导入接口回 `embeddedRegex`，前端弹一句「现在启用吗」，点启用按原件状态恢复（作者本来关掉的不会被一键打开）；运行时只读表，顺序照 ST 的 全局 → 预设 → 角色卡，显示侧正则补上预设那一档；设置页按来源分组、每组一个总开关；启动时回填（卡按原状态、预设 / 世界书默认关闭）；ST 迁移照搬 `character_allowed_regex` / `preset_allowed_regex`。**为什么要紧**：预设自带的正则以前压根没生效过，而本机 14 个 ST 预设里 11 个带（小冰块 24 条、双人成行 36 条），思维链美化 / 不发送思维链全靠它。契约见 `docs/M3-CONTRACT.md` 的「修正（2026-09-18）」。验证：服务端 5 个新测试 + 老测试改写；真机用本机真预设（ARGO-1.5 / 小冰块 V3.35）与两张真卡跑通——导入弹窗、点启用后 24 条里只开作者开着的 11 条、设置页分组显示、回填日志。 | 表里的编辑不写回原件（导出的卡 / 预设仍是作者的原版）；世界书自带正则没有真实样本，只做了「带了就收」；迁移向导没有单独汇总「自带正则」的条数（在设置页能看到）；自带脚本不支持排序（顺序跟原件）。 |

| 2026-09-22 | 收尾轮（中途收束） | 契约：M4 第二部分（背景 / 立绘 / 主题变体 / 外接生图）、M5 第二部分（变量管理器 / 脚本库 / slash / EJS）、新建 `docs/M6-CONTRACT.md`、`docs/M7-CONTRACT.md`；迁移 0003（scripts、character_sprites、writing_projects、documents、document_versions、characters.edited_at）。**已完成并验收**：①提供商工具调用与结构化输出（四家适配器 tools / tool_choice / tool_call / tool_result / 结构化输出，修掉 openai-chat 忽略 index 的 bug；`collectStream`、文本降级协议、服务端 `callLlm` / `streamLlm`；Z.AI glm-4.5-flash 两个端点原生工具调用真机跑通）；②EJS 提示词模板（ST-Prompt-Template 兼容子集，QuickJS 沙箱，200ms / 32MB 限制；黄金庭院好感度 20/50/70/95 → 阶段 01–04 验收通过；web 包不含 QuickJS）；③工作台服务端基础（角色卡新建 / 编辑 / 头像、编辑过的卡导出从 data 重写而未编辑的仍回原件、版本历史 50 版与恢复、提示库、草稿组装测试、测试会话、世界书触发模拟）。**已落地但未走完验收（代理被中途停止）**：背景库与各世界背景处理、立绘表情接口与分类、主题变体（校验与编辑器）、四种外接生图后端（只有 mock 测试）与 imagine SSE、变量管理器与 schema、全局脚本库（接口 + 设置页）、slash 解析执行器与 Composer 接入、前端卡 generate 的 injects / overrides、toast、MVU 额外模型解析、写作核心组装与服务端接口（真机 5 章：OpenAI 兼容端点后续 14/14 次命中缓存，Anthropic 兼容端点 9/14）。验证：全仓 typecheck / lint / build 通过，providers 284、core 428、compat 134、server 211、web 49、黄金 62/62 全过；隔离服务端逐页冒烟无报错、无横向溢出。 | **未做**：工作台前端（`/studio` 仍是占位页）、AI 协作者 `/api/studio/assist`（未开始）、写作前端（`/writing` 仍是占位页）；背景 / 立绘 / 变体的六世界截图审阅未做；ST 迁移的背景、立绘与脚本导入没收尾验证；glm-5.3-flash 余额不足未能真机，且它已不能关闭推理（目录已标 canDisableThinking=false）；Google / Responses 的工具调用只有回放测试。 |
| 2026-09-23 | 收尾轮续做 | **完成**：①AI 协作者 `POST /api/studio/assist`（工具循环 12 步、patch 归并、过载退避重试；glm-4.7-flash 真机一句话生成整卡通过）与预设 `PUT /:id/layout-policy`；②预设 / 世界书编辑器拆成受控组件（`features/library/{preset,lorebook}-editor`），预设拖拽排序、布局策略与保真锁，世界书触发模拟抽屉；③工作台前端 `/studio`（入口页、三栏工作台、角色卡编辑器、AI 协作 diff 逐条接受、版本、提示库、带草稿的测试对话与检查器；AI 编辑后导出 PNG 字段一致，未编辑卡字节不变）；④写作前端 `/writing`（TipTap 懒加载、续写待定区、重写对照、自动保存与版本、圣经嵌入世界书编辑器、六世界写作纸面）；⑤M4（二）/ M5（三）遗留验收：本机 ST 真迁移 69/69（背景 23、Seraphina 立绘 28、预设脚本 13），补齐五个空词典命名空间，变量管理器界面，前端卡 generate 的 user_input 顺序修正；⑥六个世界各自的背景处理与立绘框（`themes/<id>/media.css` + `SpriteFrame`）；⑦英文词典按需加载，首屏 index chunk 351.7 → 289.1 kB。全仓 typecheck / lint / test / build 通过，黄金 62/62。 | 未做 / 仅 mock：外接生图四后端、表情分类、MVU 额外模型解析只用 mock；Google / Responses 工具调用只有回放；glm-5.3-flash 余额不足；工作台导出 PNG 未在 ST 1.18 真导入（用 compat 解析代替）；预设 / 世界书的 AI 协作无真机；老库已导入预设不补写自带变量；`/bg` 无参报错。 |
