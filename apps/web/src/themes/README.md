...
| `main`                    | 路由内容容器 `main`                                                                                                                                             | —                                           |
| `route-fallback`          | 路由页面 chunk 加载中的占位（`main` 内；240ms 后才出现一行 `.pulse-live` 的「加载中…」）                                                                        | —                                           |
| `command-palette-trigger` | 顶栏的「搜索与跳转」入口按钮（窄屏只显示图标）                                                                                                                  | —                                           |
...
| `chat-list-actions`                                            | `chat-list-item` 内删除 + 导出的竖排操作组（悬停/聚焦出现，`pointer: coarse` 常显）                                     | —                                                                                       |
| `chat-import-form`                                             | 导入聊天记录弹窗里的表单（选文件 + 选角色，`modal-body` 内）                                                            | —                                                                                       |
| `chat-import-result`                                           | 导入完成后的结果块（消息数、告警）                                                                                      | —                                                                                       |
| `composer-tray`                                                | 附件托盘：`composer-dock` 内、`composer` 外框之上的一行（横向滚动，两端淡出）                                           | —                                                                                       |
| `composer-tray-hint`                                           | 托盘下的轻提示（失败项、模型看不到图片 / PDF 以文本发送）                                                               | —                                                                                       |
| `composer-attach`                                              | `composer` 里 textarea 左侧的附件键（回形针）                                                                           | —                                                                                       |
| `attachment`                                                   | 一个附件：托盘缩略图 / 文档小片、消息里的图片或文档、编辑态附件                                                         | `data-kind="image                                                                       | pdf       | text"`、`data-context="tray  | message | edit"`、`data-status`  |
| `attachment-ext` / `attachment-button` / `attachment-progress` | 文档类型字 / 移除或重试小键 / 上传进度线                                                                                | `attachment-button`：`data-tone`                                                        |
| `attachment-drop` / `attachment-drop-frame`                    | 把文件拖到对话区时出现的提示层 / 其中的框                                                                               | —                                                                                       |
| `message-media`                                                | 消息里的附件块（图片网格或文档列表）                                                                                    | `data-kind="images                                                                      | documents | edit"`、`data-layout="single | pair    | triple"`、`data-count` |
...
| `studio-aside`       | 右栏（会话 / AI 协作 / 检查器 / 版本 / 提示库页签；窄屏时是「协作」分段），`bg-panel` 面；分区上色的世界（如酒馆）要把它归到侧栏那一区，否则字色继承主区 | —                                                       |
| `studio-session`     | 右栏「会话」页签的滚动容器，里面是测试会话的 `session-panel`（与对话页右栏同一个组件）                                                                  | —                                                       |
| `studio-test-char`   | 会话面板卡面下方的「测试角色」选择（仅 preset / lorebook 的测试会话）：`.field` 下拉 + 一行提示；切换即重开测试会话                                     | —                                                       |
